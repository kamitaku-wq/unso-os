...
export async function getMonthlyKpi(includeAll = false) {
  ...
  const [thisJobs, prevJobs, thisExp, prevExp, reviewJobs] = await Promise.all([
    supabase.from('cleaning_jobs').select('amount, status, created_at, approved_at')
      .in('status', jobStatuses).gte('work_date', thisRange.start).lte('work_date', thisRange.end),
    supabase.from('cleaning_jobs').select('amount')
      .in('status', jobStatuses).gte('work_date', prevRange.start).lte('work_date', prevRange.end),
    supabase.from('expenses').select('amount').in('status', expStatuses).eq('ym', thisYm),
    supabase.from('expenses').select('amount').in('status', expStatuses).eq('ym', prevYm),
  ...
  const thisSales = sum(thisJobs.data ?? [])
  const prevSales = sum(prevJobs.data ?? [])
  const thisExpAmt = sum(thisExp.data ?? [])
  const prevExpAmt = sum(prevExp.data ?? [])
  ...
  // 件数・承認率
  const thisJobData = thisJobs.data ?? []
...
export async function getUnbilledAmount() {
  const supabase = await createClient()
  const { data, error } = await supabase.from('cleaning_jobs').select('amount')
...
export async function getMonthlySales(includeAll = false) {
  ...
  const { data, error } = await supabase.from('cleaning_jobs').select('ym, amount')
...
export async function getMonthlyExpenses(includeAll = false) {
  ...
  const { data, error } = await supabase.from('expenses').select('ym, amount').in('status', statuses).in('ym', ymList)
  if (error) throw new Error(error.message)
  const totals: Record<string, number> = Object.fromEntries(ymList.map(ym => [ym, 0]))
  for (const r of data ?? []) { if (r.ym && r.ym in totals) totals[r.ym] += Number(r.amount ?? 0) }
  return ymList.map(ym => ({ ym, amount: totals[ym] }))
...
export async function getMonthlyJobCounts(includeAll = false) {
  ...
  const ymList = getRecentYmList(6)
  const statuses = includeAll ? ['REVIEW_REQUIRED', 'APPROVED'] : ['APPROVED']
  const { data, error } = await supabase.from('cleaning_jobs').select('ym').in('status', statuses).in('ym', ymList)
  if (error) throw new Error(error.message)
...
export async function getStoreBreakdown(includeAll = false) {
  ...
  const statuses = includeAll ? ['REVIEW_REQUIRED', 'APPROVED'] : ['APPROVED']
  const { data, error } = await supabase.from('cleaning_jobs').select('store_name, amount')
...
export async function getWorkTypeBreakdown(includeAll = false) {
  ...
  const { start, end } = ymToRange(thisYm)
  const statuses = includeAll ? ['REVIEW_REQUIRED', 'APPROVED'] : ['APPROVED']
  const { data, error } = await supabase.from('cleaning_jobs').select('work_name, amount')
    .in('status', statuses).gte('work_date', start).lte('work_date', end)
  if (error) throw new Error(error.message)
...
export async function getExpenseCategoryBreakdown(includeAll = false) {
  ...
  const { thisYm } = getCurrentAndPrevYm()
  const statuses = includeAll ? ['SUBMITTED', 'APPROVED', 'PAID'] : ['APPROVED', 'PAID']
  const { data, error } = await supabase.from('expenses').select('category_name, amount')
...
export async function getAttendanceSummary() {
  ...
  const { thisYm } = getCurrentAndPrevYm()
  const { start, end } = ymToRange(thisYm)
  const { data, error } = await supabase.from('attendances').select('work_min, overtime_min, emp_id')
