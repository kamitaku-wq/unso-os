...
export async function exportBillables(periodFrom?: string, periodTo?: string) {
  ...
  const headers = ['実績ID', '運行日', '社員ID', '荷主コード', 'ルートコード', '積み地', '降ろし地', '出発時刻', '到着時刻', '距離(km)', '金額', 'ステータス', '請求書番号', '備考']
  const rows = (data ?? []).map(r => [
    r.billable_id, r.run_date, r.emp_id, r.cust_id, r.route_id,
    r.pickup_loc, r.drop_loc, r.depart_at, r.arrive_at,
    r.distance_km, r.amount, r.status, r.invoice_id, r.note,
  ])
...
export async function exportExpenses(ym?: string) {
  ...
  const headers = ['経費ID', '経費日', '社員ID', '年月', '区分', '金額', '支払先', '内容', 'ステータス', '承認日', '支払日']
  const rows = (data ?? []).map(r => [
    r.expense_id, r.expense_date, r.emp_id, r.ym,
    r.category_name, r.amount, r.vendor, r.description,
...
export async function exportAttendances(ym?: string) {
  ...
  const headers = ['勤怠ID', '勤務日', '社員ID', '年月', '出勤時刻', '退勤時刻', '休憩(分)', '勤務時間(分)', '残業(分)', '運転時間(分)', 'ステータス']
  const rows = (data ?? []).map(r => [
    r.attendance_id, r.work_date, r.emp_id, r.ym,
    r.clock_in, r.clock_out, r.break_min,
